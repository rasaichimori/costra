...
export const getConversionFactor = (
	...
	for (const conv of conversions) {
		if (conv.ingredientId === ingredientId) {
			// Schema: "how many inputs are in one output"
			// Example: inputUnit="g", outputUnit="cup", factor=165 means "165g = 1 cup"
			// So: factor inputs = 1 output
			// To convert inputUnit → outputUnit: outputAmount = inputAmount / factor
			// To convert outputUnit → inputUnit: inputAmount = outputAmount × factor
			// Add forward direction: inputUnit → outputUnit (divide by factor)
			addEdge(conv.inputUnit, conv.outputUnit, 1 / conv.conversionFactor);
			// Add reverse direction: outputUnit → inputUnit (multiply by factor)
			addEdge(conv.outputUnit, conv.inputUnit, conv.conversionFactor);
		}
	}
